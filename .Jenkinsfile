    ...
    stages {
        stage('Resolve TAO dependencies') {
            environment {
                ...
                REPO_NAME='oat-sa/extension-tao-funcacl'
            }
            steps {
                sh(
        ...
        stage('Tests') {
            parallel {
                stage('Backend Tests') {
                    when {
                        expression {
                            fileExists('build/funcAcl/test/unit')
                        }
                    }
                    agent {
                        docker {
                    ...
                    steps {
                        dir('build'){
                            sh(
                                ...
                                script: './vendor/bin/phpunit funcAcl/test/unit'
                            )
                        }
                    }
                }
                stage('Frontend Tests') {
                    when {
                        expression {
                            fileExists('build/funcAcl/views/build/grunt/test.js')
                        }
                    }
                    agent {
                        docker {
                    ...
                    steps {
                        dir('build/tao/views/build') {
                            ...
                            sh (
                                ...
                                script: 'npx grunt connect:test funcacltest'
                            )
                        }
                    }
                }
